...
async function markAsMyRecipes(user_id, username, recipe_id, title, image, how_to_make, time_to_make, likes, is_vegan, is_gluten_free, servings){
    await DButils.execQuery(`insert into myrecipes values ('${user_id}','${username}',${recipe_id},'${title}','${image}','${how_to_make}','${time_to_make}','${likes}','${is_vegan}','${is_gluten_free}','${servings}')`);
}
